...
class PyFunction(FunctionObject):
    ...
    def __init__(self):
        ...
        self.id = 'FC_GET_SKILL_LEVEL'
        self.country = 'CHN'
        self.desc = '获取技能等级函数'
        self.descENG = '获取技能等级函数'
        self.func_type = 'A'
        self.instructions = "获取技能等级函数。输入参数：日期。"
        ...
        if isinstance(to_date, str):
            try:
                ...
                try:
                    ...
                    raise Exception("函数FC_GET_SKILL_LEVEL的参数错误")
        elif not isinstance(to_date, datetime.date):
            raise Exception("函数FC_GET_SKILL_LEVEL的参数错误")

        db = gv.get_db()
        catalog = gv.get_run_var_value('PY_CATALOG')
        t = text("select hhr_skill_level from boogoo_payroll.hhr_py_skill_level where tenant_id = :b1 and hhr_empid = :b2 "
